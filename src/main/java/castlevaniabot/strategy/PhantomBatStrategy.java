...
public class PhantomBatStrategy extends Strategy {
  ...
  public PhantomBatStrategy(final CastlevaniaBot b) {
    super(b);
  }
  ...
  @Override public void step() {
    ...
    if (b.hearts >= 5 && b.weapon == STOPWATCH) {
      ...
      switch(b.weapon) {
        case HOLY_WATER: stepHolyWaterStrategy(bat);                   break;
        case AXE:        stepAxeStrategy(bat, offsetX, offsetY);       break;
        case DAGGER:     stepDaggerStrategy(bat, offsetX, offsetY);    break;        
        default:         stepNoWeaponsStrategy(bat, offsetX, offsetY); break;
      }      
    } else {
    ...
    if (axeScanCounter > 0) {
      ...
      for(int i = AXE_SCANS.length - 1; i >= 0; --i) {
        ...
        if (b.canHitTargetWithAxe(platformX, 13)) {
          routeX = platformX << 4;
          switch(AXE_SCANS[i]) {
            case 0:  routeX += 10; break;
            case 9:  routeX +=  6; break;
